...
from .reaction_pattern import reaction_pattern
...
def does_message_contain_patterns(
    patterns: list[str], message: discord.Message
) -> bool:
    """
...
async def handle_reaction(
    ...
    for r in usable_reactions:
        logger.debug(
            f'Adam is looking if he can react to message "{message.content}" with {r.emoji}'
        )
...
def __get_usable_reactions__(
    emojis: list[discord.Emoji], reactions
) -> list[reaction_pattern]:
    usable_reactions = []
    for e in emojis:
        for r in reactions:
            if e.name == r["emoji"]:
...
def load_reactions_db(
    ondra: discord.Client, logger: logging.Logger
) -> dict[discord.Guild, list[reaction_pattern]]:
    db = {}
    reactions = load_reactions_from_file(logger=logger)

    for guild in ondra.guilds:
        logger.debug(
            f'Adam is loading the usable reactions for the "{guild.name}" discord server'
        )
...
def load_server_reactions(
    emojis: list[discord.Emoji], logger: logging.Logger
) -> list[reaction_pattern]:
